...
function computeCCI(highs, lows, closes, period) {
    ...
    const meanDeviation =
        typicals.reduce((acc, tp) => acc + Math.abs(tp - ma), 0) / period
...
function minimax(values, period, normMin, normMax) {
...
export function processKlines(klines, options) {
    ...
    const trainingFeature1 = []
    const trainingFeature2 = []
